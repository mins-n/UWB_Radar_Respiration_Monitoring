...
BIOPAC_path = "./../Data/2022.12.27/2022.12.27_2_sun_jin/2022.12.27_2_sun_jin.mat"
sample_count = 0
...
Window_rawdata = np.array(rawdata[:, :])
SD = np.array([])
...
diff = 9999
for i in range(0,len(UWB_rpeak_i[0])//4-5) :
    for j in range(0,len(data1_rpeak_i)//4-5) :
        tmp = 0
        for k in range(0,4) :
...
if sync >= 0:
    ...
    plt.plot(data1[show-sync:show+500 ])
    plt.gca().axes.xaxis.set_visible(False)
    plt.gca().axes.yaxis.set_visible(False)
    plt.show()
    
    BIOPAC_sync.append(data1[-sync:])
    BIOPAC_sync.append(data1_fs)
    ...
    plt.plot(data2[show-sync:show+500 ])
    plt.gca().axes.xaxis.set_visible(False)
    plt.gca().axes.yaxis.set_visible(False)
    plt.show()
    
    BIOPAC_sync.append(data2[-sync:])
    BIOPAC_sync.append(data2_fs)
